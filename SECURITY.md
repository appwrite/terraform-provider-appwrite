...
Those SBOMs are generated by scanning the archives the release originally
published, so they describe what actually shipped rather than what a rebuild
would produce today.
...
$ sha256sum --check terraform-provider-appwrite_<version>_SBOMS_SHA256SUMS
```
