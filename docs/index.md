...
- `max_retries` (Number) How many times to retry a request that failed for a reason likely to pass, such as a rate limit or a gateway error. Defaults to 5. Set to 0 to disable retrying. Note that Terraform applies resources in parallel, so raising both this and -parallelism against a rate-limited endpoint makes throttling more likely, not less.
