...
func NewSpecificationsDataSource(product Product) func() datasource.DataSource {
	return func() datasource.DataSource {
		return &specificationsDataSource{product: product}
	}
}
...
func (d *specificationsDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = fmt.Sprintf("%s_%s_specifications", req.ProviderTypeName, d.product)
}
...
func (d *specificationsDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: fmt.Sprintf(
			"Lists the compute specifications available for Appwrite %s databases. This deployment may have no shared pool, in which case `specification` is required when creating a database, so a `specification` slug can be selected "+
...
func (d *specificationsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	list, err := apiFor(d.clients, d.product, projectID).ListSpecifications()
	if err != nil {
